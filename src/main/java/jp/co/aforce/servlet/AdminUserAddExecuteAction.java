...
public class AdminUserAddExecuteAction extends Action {
	...
	public String execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (memberId == null || memberId.trim().isEmpty() ||
			...
			request.setAttribute("errorMessage", "必須項目が入力されていません。すべての項目を正しく入力してください。");
		...
		if (!password.equals(passwordConfirm)) {
			...
			request.setAttribute("errorMessage", "新規パスワードと確認用パスワードが一致しません。");
			request.setAttribute("errorBackUrl", "AdminAddForm.action");
			request.setAttribute("errorBtnText", "管理者登録画面へ戻る");
			return "login-error.jsp";
		}
		...
		if (!loginAdmin.getPassword().equals(currentAdminPassword)) {
			request.setAttribute("errorTitle", "本人確認エラー");
		...
		try {
			...
			if (dao.checkDuplicate(memberId)) {
				request.setAttribute("errorTitle", "登録エラー");
				request.setAttribute("errorMessage", "このログインIDはすでに登録されています。別のIDを指定してください。");
				request.setAttribute("errorBackUrl", "AdminAddForm.action");
				request.setAttribute("errorBtnText", "管理者登録画面へ戻る");
				return "login-error.jsp";
			}
			...
			if (count > 0) {
				...
				request.setAttribute("errorMessage", "データベースへの登録に失敗しました。時間をおいて再度お試しください。");
				request.setAttribute("errorBackUrl", "AdminAddForm.action");
				request.setAttribute("errorBtnText", "管理者登録画面へ戻る");
				return "login-error.jsp";
			}
